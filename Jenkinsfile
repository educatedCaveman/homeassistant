pipeline {
    ...
    environment {
        ...
        PORTAINER_DEV_WEBHOOK = credentials('PORTAINER_WEBHOOK_DEV_HOMEASST')
        PORTAINER_PRD_WEBHOOK = credentials('PORTAINER_WEBHOOK_PRD_HOMEASST')
    }
    ...
    stages {
        ...
        stage('deploy dev code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_dev.yml --extra-vars repo="homeassistant"'
            }
        }
        ...
        stage('deploy prd code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_prd.yml --extra-vars repo="homeassistant"'
            }
        }
        // trigger portainer redeploy
        // separated out so this only gets run if the ansible playbook doesn't fail
